...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function returnItem($orderItemId, $qty)
    {
        $itemModel = $this->_quoteCollection->create()
                                ->addFieldToFilter('order_item_id', $orderItemId)
                                ->addFieldToFilter('is_returned', 0)
                                ->getFirstItem();
        if ($itemModel->getId()) {
            $childAssignId = $itemModel->getChildAssignId();
            if ($childAssignId) {
                $associate = $this->_associates->create()->load($childAssignId);
                $associate->setQty($associate->getQty()+$qty);
                $associate->save();
            } else {
                $assignId = $itemModel->getAssignId();
                $item = $this->_items->create()->load($assignId);
                $item->setQty($item->getQty()+$qty);
                $item->save();
            }
            $itemModel->setIsReturned(1);
            $itemModel->save();
        }
    }

    public function checkQuoteValidity()
    {
        $quote = $this->_cart->getQuote();
        $error = false;
        foreach ($quote->getAllItems() as $item) {
            if (!$item->getParentItemId()) {
                $assignedQuotes = $this->quoteFactory->create()
                                            ->getCollection()
                                            ->addFieldToFilter('item_id', $item->getId())
                                            ->addFieldToFilter('quote_id', $quote->getId())
                                            // ->addFieldToFilter('product_id', $item->getProductId())
                                            ->getFirstItem();
                if ($assignedQuotes->getId()) {
                    $assignedItem = $this->_items->create()
                                            ->getCollection()
                                            ->addFieldToFilter('seller_id', $assignedQuotes->getSellerId())
                                            ->addFieldToFilter('product_id', $assignedQuotes->getProductId())
                                            ->addFieldToFilter('qty', ['lteq'=>$assignedQuotes->getQty()])
                                            ->getFirstItem();
                    ...
                    if (!$assignedItemId) {
                        ...
                        $product = $this->productRepository->getById($assignedQuotes->getProductId());
                        if ($product->getTypeId()=='configurable') {
                            $assignedAssociateItem = $this->associatesFactory->create()
                                                            ->getCollection()
                                                            ->addFieldToFilter('parent_id', $assignedItemId)
                                                            ->addFieldToFilter('id', $assignedQuotes->getChildAssignId())
                                                            ->getFirstItem();
                            if (!$assignedAssociateItem->getId()) {
                                $error = true;
                            }
                            if ($assignedAssociateItem->getQty()<$item->getQty()) {
                                $error = true;
                            }
                        } else {
                            if ($assignedItem->getQty()<$item->getQty()) {
                                $error = true;
                            }
                        }
                    }
    ...
    {
        $assignId = 0;
        $assignedProduct = $this->getCollection()
                            ->addFieldToFilter('product_id', $productId)
                            ->addFieldToFilter('seller_id', $sellerId)
                            ->setPageSize(1)->setCurPage(1)->getFirstItem();
        return $assignedProduct->getId() ? $assignedProduct->getId() : 0;
    }
    ...
    public function getOriginalQty($productId, $type = "")
    {
        ...
        if ($type == "configurable") {
            $collection = $this->_associates->create()
                                ->getCollection()
                                ->addFieldToFilter('product_id', $productId);
    ...
    public function getAssignProducts($productId, $sort = '', $order = 'ASC', $exclude = false)
    {
        ...
        $collection->getSelect()->joinLeft(
            ...
            'main_table.assign_product_id = cped.row_id and cped.store_id = 0
            ...
            foreach ($collection as $assignProduct) {
                ...
                if ($mode == 0) {
                    ...
                    if ($product->getId()) {
                        ...
                        $productData['price'] = (float) $product->getPrice();
    ...
    public function getAssociatesData($assignId)
    {
        ...
        $collection->getSelect()->joinLeft(
            $catalogProductEntityDecimal.' as cped',
            'main_table.product_id = cped.row_id and cped.store_id = 0
            AND cped.attribute_id = '.$proPriceAttrId,
            ["product_price" => "value"]
        );
        foreach ($collection as $item) {
            $info = [
                    ...
                    'product_price' => number_format($item->getProductPrice(), 2),
                    'product_id' => $item->getAssignProductId()
                ];
    ...
    public function getMinimumPriceProducts($productId)
    {
        if ($this->showMinimumPrice()) {
            ...
            $productCollection = $this->getProductCollection()->addAttributeToSelect('price')
                                        ->addFieldToFilter('entity_id', ['in' => $assignProductIds]);
            $productCollection->addAttributeToSort('price', 'ASC');
            return $productCollection->setPageSize(1)->setCurPage(1)->getFirstItem()->getId();
        } else {
            return false;
        }
        return false;
    ...
    {
        $formattedPrice = $this->currencyModel->format($price, ['display'=>\Zend_Currency::NO_SYMBOL], false);
        return $price;
    }
    ...
    {
        $config = 'carriers/delhivery/';
        return [
            'seller_set_qty' => $this->_scopeConfig->getValue(
                $config.'seller_set_qty',
                \Magento\Store\Model\ScopeInterface::SCOPE_STORE
            ),
            'seller_set_price' => $this->_scopeConfig->getValue(
                $config.'seller_set_price',
                \Magento\Store\Model\ScopeInterface::SCOPE_STORE
            )
        ];
    }
    ...
    /**
     * Checking product type using product Id
     *
     * @param  int $productId
     *
     */
