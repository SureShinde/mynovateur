...
class Save extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        $sellerAccConfig = $helper->getSellerAccConfig();
        if ($productType=='simple') {
            $collection = $this->itemsFactory->create()
                                    ->getCollection()
                                    ->addFieldToFilter('seller_id', $sellerId)
                                    ->addFieldToFilter('id', $data['assign_id']);
            foreach ($collection as $model) {
                if (!empty($sellerAccConfig) && $sellerAccConfig['seller_set_qty']){
                    $model->setQty($data['qty'] ?? 0);
                }
                if (!empty($sellerAccConfig) && $sellerAccConfig['seller_set_price']){
                    $model->setPrice($data['price'] ?? $product->getPrice());
                }
                if (!empty($sellerAccConfig) && ($sellerAccConfig['seller_set_qty'] || $sellerAccConfig['seller_set_price'])) {
                    $model->save();
                    $this->messageManager->addSuccess(__('Product is saved successfully.'));
                } else {
                    $this->messageManager->addNotice(__('Product information update disabled from admin.'));
                }
            }
        } else if(isset($data['products'])) {
            foreach ($data['products'] as $childProduct) {
                $collection = $this->associatesFactory->create()
                                        ->getCollection()
                                        ->addFieldToFilter('id', $childProduct['associate_id'])
                                        ->addFieldToFilter('parent_id', $data['assign_id']);
                foreach ($collection as $model) {
                    if (!empty($sellerAccConfig) && $sellerAccConfig['seller_set_qty']){
                        $model->setQty($childProduct['qty'] ?? 0);
                    }
                    if (!empty($sellerAccConfig) && $sellerAccConfig['seller_set_price']){
                        $model->setPrice($childProduct['price'] ?? 0);
                    }
                    if (!empty($sellerAccConfig) && ($sellerAccConfig['seller_set_qty'] || $sellerAccConfig['seller_set_price'])) {
                        $model->save();
                    }
