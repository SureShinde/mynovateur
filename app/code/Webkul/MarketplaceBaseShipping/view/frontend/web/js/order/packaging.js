...
define([
    ...
    'mage/template',
    'text!Webkul_DelhiveryShipping/templates/pickup-popup.html'
], function (prototype, jk, calendar, confirm, alert, template, pickuptemplate) {
    ...
    Packaging.prototype = {
        ...
        initialize: function (params) {
            ...
            this.defaultItemsQty = params.defaultItemsQty ? params.defaultItemsQty : null;
            this.shipmentItemsQty = params.shipmentItemsQty ? params.shipmentItemsQty : null;
        ...
        sendCreateLabelRequest: function () {
            ...
            console.log(this.createLabelUrl);
            this.messages.hide().update();
            ...
            if (this.createLabelUrl) {
                ...
                jk('<div />').html(modalHtml)
                .modal({
                    title: jk.mage.__('Enter Pickup Time'),
                    autoOpen: true,
                    opened: function(){
                         jk("#pickup_time").calendar({
                    ...
                    closed: function () {
                        jk('.modals-wrapper').remove();
                    },
                    buttons: [{
                     text: jk.mage.__('Set Pickup Time'),
                        attr: {
                            'data-action': 'button'
                        },
                        'class': 'action-primary',
                        click: function() {
                            var pickupTime = jk('#pickup_time').val();
                            if (pickupTime) {
                                self.shipmentProcess(self, pickupTime);
                                this.closeModal();
                            } else {
                                jk('#pickup_time').css({"border":"1px solid rgb(255, 0, 0)","width":"90%"});
                                return false;
                            }
                        }
                    }]
        ...
        shipmentProcess: function (self, pickupTime) {
            ...
            this.packagesContent.childElements().each(function (pack) {
                ...
                noOfPacket = parseFloat(pack.select('input[name="no_of_packet"]')[0].value);
                packagesParams[packageId] = {
                    ...
                    noOfPacket: isNaN(noOfPacket) ? 1 : noOfPacket
                };
            ...
            for (var packageId in this.packages) {
                if (!isNaN(packageId)) {
                    ...
                    this.paramsCreateLabelRequest['packages[' + packageId + ']' + '[params]' + '[no_of_packet]'] = packagesParams[packageId]['noOfPacket'];
                    this.paramsCreateLabelRequest['packages[' + packageId + ']' + '[params]' + '[pickup_time]'] = pickupTime;
        ...
        validate: function () {
            var dimensionElements = $('packaging_window').select(
                'input[name=no_of_packet],input[name=container_length],input[name=container_width],input[name=container_height],input[name=container_girth]:not("._disabled")'
            );
            var callback = null;
        ...
        validateItemQty: function (itemId, qty) {
            ...
            return this.defaultItemsQty[itemId] - this.shipmentItemsQty[itemId];
        },
        ...
        _setAllItemsPackedState: function () {
            ...
            if (this._getItemsCount(this.getPackedItemsQty())>0
