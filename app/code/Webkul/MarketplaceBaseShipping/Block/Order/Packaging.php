...
class Packaging extends \Magento\Framework\View\Element\Template
{
    ...
    public function getConfigDataJson()
    {
        ...
        $fullUrl = $route.'/'.$controller.'/'.$action;

        $order = $this->getOrder();
        if (in_array($fullUrl, ['marketplace/order/view', 'marketplace/order_shipment/lists']) && $order->canShip() && $order->hasShipments()) {
            if ($orderId) {
                ...
                foreach ($order->getAllItems() as $item) {
                    $itemsQty[$item->getId()] = $item->getQtyOrdered() * 1;
                    $itemsShippedQty[$item->getId()] = $item->getQtyShipped();
                    ...
                    $itemsOrderItemId[$item->getId()] = $item->getId();
                }
            }
        } else if ($shipmentId) {
            ...
            if ($orderId) {
                ...
                foreach ($this->getShipment()->getAllItems() as $item) {
                    ...
                    $itemsShippedQty[$item->getOrderItemId()] = 0;
        ...
        $data = [
            ...
            'defaultItemsQty' => $itemsQty,
            'shipmentItemsQty' => $itemsShippedQty,
