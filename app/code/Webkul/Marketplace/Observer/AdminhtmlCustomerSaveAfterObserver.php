...
class AdminhtmlCustomerSaveAfterObserver implements ObserverInterface
{
    ...
    public function assignProduct($sellerId, $productIds)
    {
        ...
        $sellerProductIds = $this->itemsFactory->create()
                                    ->getCollection()
                                    ->addFieldToFilter('seller_id', $sellerId)
        ...
        foreach ($additionalProductIds as $additionalProductId) {
            ...
            $item->setSellerId($sellerId)
                    ->setProductId($additionalProductId)
                    ->save();
            
            $curProduct = $this->productModel->create()->load($additionalProductId);
            if ($curProduct->getTypeId()=='configurable') {
                $_children = $curProduct->getTypeInstance()->getUsedProducts($curProduct);
                foreach ($_children as $child) {
                    $associates = $this->associatesFactory->create();
                    $associates->setParentId($item->getId())
                                ->setProductId($child->getId())
                                ->setParentProductId($additionalProductId)
                                ->save();
                }
            }
        }
    ...
    public function unassignProduct($sellerId, $productIds)
    {
        ...
        if (!empty($productIds)) {
            ...
            foreach ($collection as $coll) {
                $associates = $this->associatesFactory->create()
                                    ->getCollection()
                                    ->addFieldToFilter('parent_id', $coll->getId());
