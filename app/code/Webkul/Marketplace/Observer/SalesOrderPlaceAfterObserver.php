        ...
        if (!$item->getParentItemId()) {
            ...
            $assignedQuotes = $this->quoteFactory->create()
                                        ...
                                        ->addFieldToFilter(
                                            ...
                                            [
                                                ['eq' => $item->getQuoteItemId()],
                                                ['eq' => $item->getQuoteItemId()]
                                            ]
                                        )
                                        // ->addFieldToFilter('product_id', $item->getProductId())
                                        ->getFirstItem();
            $sellerId = $assignedQuotes->getSellerId();
            if ($sellerId) {
                $assignedQuotes->setOrderId($item->getOrderId());
                $assignedQuotes->setOrderItemId($item->getId());
                $assignedQuotes->save();
            }
            if ($assignedQuotes && $assignedQuotes->getId()) {
                ...
                if ($assignedItemId) {
                    ...
                    $product = $this->_productRepository->getById($assignedQuotes->getProductId());
                    if ($product->getTypeId()=='configurable') {
                        $assignedAssociateItem = $this->associatesFactory->create()
                                                        ...
                                                        ->getFirstItem();
                        $qty = $assignedAssociateItem->getQty()-$item->getQtyOrdered();
                        $assignedAssociateItem->setQty($qty);
                        $assignedAssociateItem->save();
                    } else {
                        $qty = $assignedItem->getQty()-$item->getQtyOrdered();
                        $assignedItem->setQty($qty);
                        $assignedItem->save();
                    }
                }
            }
