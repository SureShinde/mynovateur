...
class Invoice extends \Webkul\Marketplace\Model\Order\Pdf\Invoice
{
    ...
    protected $_string;
    /**
     * @var \Magento\Framework\Model\Order
     */
    protected $_order;
     ...
     * @param \Webkul\MarketplaceGstIndia\Model\Order\Pdf\Invoice $invoice
    ...
    public function __construct(
        ...
        \Webkul\MarketplaceGstIndia\Model\Order\Pdf\Invoice $invoice,
    ...
    {
        if (!$this->_gstHelper->getConfigValue('status')) {
        ...
        foreach ($sellerinvoices as $sellerinvoice) {
            ...
            $this->invoice->_order = $sellerOrder;
            /* Add image */
            $this->insertLogo(
                $sellerPage,
            ...
            if ($this->_gstHelper->getConfigValue('status') &&
                $this->_gstHelper->getCountryFromOrder($sellerOrder) == 'IN'
            ) {
                $seller = $this->helper->getCustomer();
                if ($seller) {
                    // $this->invoice->insertGstNumber($sellerPage, __('GSTIN # 123') . $seller->getSellerGstin());
                }
            }
            $itemCount = 0;
            ...
            /* Add table */
            if ($sellerinvoice->getShippingAddress() == null) {
                $this->invoice->_drawHeader($sellerPage, $this->y + 25, $itemCount);
            } else {
                $this->invoice->_drawHeader($sellerPage, $this->y + 27, $itemCount);
            }
            /* Add body */
            foreach ($sellerinvoice->getAllItems() as $item) {
