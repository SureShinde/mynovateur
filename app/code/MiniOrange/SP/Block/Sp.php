...
class Sp extends \Magento\Framework\View\Element\Template
{
    ...
    {
        $amCountry = $this->spUtility->getStoreConfig(SPConstants::MAP_COUNTRY);
        return !$this->spUtility->isBlank( $amCountry) ?  $amCountry : '';
    }
    ...
    {
        $amCity = $this->spUtility->getStoreConfig(SPConstants::MAP_CITY);
        return !$this->spUtility->isBlank($amCity) ?  $amCity : '';
    }
...
/**
    ...
    */
     ...
     public function getAddressMapping()
          {
              $amAddress = $this->spUtility->getStoreConfig(SPConstants::MAP_ADDRESS);
              return !$this->spUtility->isBlank($amAddress) ?  $amAddress : '';
          }
    ...
    */
     ...
     {
         $amPhone = $this->spUtility->getStoreConfig(SPConstants::MAP_PHONE);
         return !$this->spUtility->isBlank($amPhone) ?  $amPhone : '';
     }
    ...
    public function getCompanyIdMapping()
    {
        $amCompanyId = $this->spUtility->getStoreConfig(SPConstants::MAP_COMPANY_ID);
        return !$this->spUtility->isBlank( $amCompanyId) ?  $amCompanyId : '';
    }
   ...
   /**
       * Get all custom attributes set by the admin on his site.
       */

     public function getCustomMapping()
       {
           $amCustomName = array();
           $amCustomName = $this->spUtility->getStoreConfig(SPConstants::CUSTOM_MAPPED);
           return !$this->spUtility->isBlank( $amCustomName) ?  json_decode($amCustomName) : '';
           $this->spUtility->log_debug("saved arrays ",$amCustomName);
 ...
 /**
     * Get all customer attributes set by the admin on his site.
     */
    ...
    /**
     * Get the default group to be set for the frontend user if it
     * doesn't match any of the group mappings
     */
    public function getDefaultGroup()
    {
        $defaultGroup = $this->spUtility->getStoreConfig(SPConstants::MAP_DEFAULT_GROUP);
        return !$this->spUtility->isBlank( $defaultGroup) ?  $defaultGroup : SPConstants::DEFAULT_GROUP;
    }
    ...
    public function getUpdateMapping()
    {
        $amUpdate = $this->spUtility->getStoreConfig(SPConstants::ATTRIBUTE_UPDATE);
        return !$this->spUtility->isBlank($amUpdate) ? $amUpdate: '';
    }
