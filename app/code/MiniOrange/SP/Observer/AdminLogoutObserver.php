...
class AdminLogoutObserver implements ObserverInterface
{
    ...
    {
		try{
            $this->spUtility->log_debug("In Admin Logout Observer");
			...
			{	// check if user data has been set info for logout
                $this->spUtility->log_debug("In Admin Logout Observer: if userdetails blank && isUserLoggedin");
				$data['admin'] = TRUE;
			...
			if(!$this->spUtility->isBlank($userDetails))
                $this->spUtility->log_debug("In AdminLogout Observer before sending request");
