...
], function (_, registry, utils, Collapsible, confirm, alert, $t, template, $, loader, calendar, pickuptemplate) {
    'use strict';
    ...
    return Collapsible.extend({
        ...
        defaultCallback: function (action, data) {
            ...
            if (action.type == 'pickup') {
                this.openPickupModel(action, selections.selected);
            } else {
                utils.submit({
                    url: action.url,
                    data: selections
         ...
         * custom work
         */
        openPickupModel: function (action, selections){
            ...
            $('<div />').html(modalHtml)
            .modal({
                title: $.mage.__('Enter Pickup Time'),
                autoOpen: true,
                opened: function(){
                     $("#pickup_time").calendar({
                ...
                buttons: [{
                 text: $.mage.__('Request Pickup'),
                    ...
                    click: function() {
                        ...
                        if (pickupTime) {
                            $('#pickup_time').removeAttr('style');
                            self.schedulePickup(action, selections);
                            this.closeModal();
                        } else {
                            $('#pickup_time').css('border', '1px solid rgb(255, 0, 0)');
                            return false;
                        }
                    }
                }]
         ...
         * schedulePickup
         */
        schedulePickup : function (action, selections) {
             $.ajax({
                 url     :   action.url,
                 type    :   "POST",
                 dataType:   "json",
